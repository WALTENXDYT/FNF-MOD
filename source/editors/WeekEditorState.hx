...
class WeekEditorState extends MusicBeatState
{
	...
	function updateText()
	{
		...
		for (i in 0...stringThing.length)
		{
			...
			else if (weekFile.shownInTracks)
				txtTracklist.text += (stringThing[i].startsWith(CoolUtil.invisibleSongPrefix) ? stringThing[i].substring(CoolUtil.invisibleSongPrefix.length,
					stringThing[i].length) : stringThing[i])
					+ '\n';
		}
